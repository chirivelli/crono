...
export function MobileNavigationBar() {
  ...
  return (
    <View className="px-5 pb-5 pt-2">
      <View className="flex-row items-center justify-around rounded-[28px] border border-gray-200 bg-white px-3 py-2">
        {items.map(item => {
          const active = isActiveRoute(item.href, pathname);

          return (
            <Link key={item.href} href={item.href} asChild>
              <Pressable className="min-w-20 items-center gap-1 rounded-2xl px-3 py-2">
